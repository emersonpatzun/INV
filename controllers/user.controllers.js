...
async function createUser(req, res) {
  ...
  if (
    ...
    try {
      ...
      if (userExists)
        res.status(400).send({ message: UserMessages.EXISTING_USER });
      else {
        user.name = data.name;
        ...
        if (data.password.match(blankSpace))
          res.send({ mesagge: UserMessages.PASSWORD_WITHOUT_SPACE });
        else {
          BCrypt.hash(data.password, null, null, async (error, password) => {
            if (error) {
              res
                .send(500)
                .send({ message: UserMessages.PASSWORD_CREATION_ERROR });
              console.log(error);
            } else {
              ...
              if (!userSaved)
                res.send({ mesagge: UserMessages.USER_CREATION_ERROR });
              else {
                res.send(userSaved);
              }
            }
  ...
  try {
    ...
    if (!userExists) res.status(400).send({ message: UserMessages.WRONG_ID });
      ...
      if (hasTransactions) {
        ...
        if (!userDeleted)
          res.send({ message: UserMessages.CANNOT_DELETE_USER });
        else {
...
async function listUsers(req, res) {
  try {
    ...
    if (!users) res.send({ message: UserMessages.IMPOSSIBLE_TO_OBTAIN_USERS });
      ...
      if (users.length == 0)
        res.send({ message: UserMessages.USERS_NOT_AVAILABLE });
      else {
        res.send(users);
      }
    }
  ...
  if (data.name || data.lastName || data.email || data.userName) {
    try {
      ...
      if (userExists)
        ...
        if (!userUpdated)
          res.send({ mesagge: UserMessages.COULD_NOT_EDIT_USER });
